...
def create_features(clean_df):
    '''Takes in the clean df and creates features of the various lights and corresponding sleep efficiency'''
    
    df = clean_df
    
    df['interval_num'] = clean_df.interval.replace({'ACTIVE': 3, 'REST': 1, 'REST-S': 0}) # so the differences are unique
    ...
    # indicies where ACTIVE (3) starts or REST-S (0) starts
    ...
    final_df = pd.DataFrame({"dim_white": awake_whitelight_series.reset_index(drop=True), 
                             "dim_blue": awake_bluelight_series.reset_index(drop=True),
                             "dim_green": awake_greenlight_series.reset_index(drop=True),
                             "dim_red": awake_redlight_series.reset_index(drop=True),
                             "sleep_eff": sleep_eff})
...
def build_features(clean_data_dir, outpath):
    '''Takes in directory of clean dfs, builds features, concatenates them and saves the final df as outfile.csv'''
    ...
    for f in files:
        if f.endswith(".csv"):
            try:
                proc_data = create_features(pd.read_csv(os.path.join(clean_data_dir, f)))
            except:
                continue 
            else:
